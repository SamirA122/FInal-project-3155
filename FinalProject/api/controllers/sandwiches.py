...
def read_all(db: Session, category: str = None, is_available: bool = None):
    try:
        query = db.query(model.Sandwich)
        if category:
            query = query.filter(model.Sandwich.category == category)
        if is_available is not None:
            query = query.filter(model.Sandwich.is_available == is_available)
        result = query.all()
    except SQLAlchemyError as e:
        error = str(e.__dict__['orig'])
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error)
